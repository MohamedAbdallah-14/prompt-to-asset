...
**Why Google dropped off the free-API list** (verified against [ai.google.dev/gemini-api/docs/pricing](https://ai.google.dev/gemini-api/docs/pricing), Apr 2026). In December 2025 Google removed all image-generation models from the universal Gemini free API tier. `gemini-3.1-flash-image-preview` (Nano Banana 2), `gemini-3-pro-image-preview` (Nano Banana Pro), `gemini-2.5-flash-image` (original Nano Banana), and all `imagen-4.0-*` variants now show `Free Tier: Not available` on Google's own pricing page and require a billing-enabled GCP project. An unbilled `GEMINI_API_KEY` returns HTTP 429 with `limit: 0` on image endpoints — that's a daily quota of zero, not a rate limit. The key still works on the free tier for text / multimodal / embeddings (subject to RPD/RPM/TPM caps), just not for image generation.
