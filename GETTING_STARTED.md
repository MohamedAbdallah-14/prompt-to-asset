...
6. **Google AI Studio (paste-only)** — the web UI at https://aistudio.google.com is free for interactive Nano Banana / Imagen generation. Useful when you want Nano Banana quality without enabling billing (see Imagen 4 note below for the free programmatic path). Generate in the UI, download the PNG, call `asset_ingest_external` to bring it into the pipeline.
...
**The Gemini free-API picture is split** (verified 2026-04-26 against the AI Studio rate-limit dashboard).

- **Imagen 4 has a free tier.** `imagen-4.0-generate-preview`, `imagen-4.0-fast-generate-preview`, and `imagen-4.0-ultra-generate-preview` each show 0/25 RPD on the free tier (RPM/TPM dashes — only daily request count is capped). Total ~75 free images/day across the Imagen 4 line on a single project. `GEMINI_API_KEY` against any of these works without billing; Imagen 4 Fast is the cheapest free Gemini illustration route.
- **Nano Banana family is paid-only.** `gemini-3.1-flash-image-preview` (Nano Banana 2), `gemini-3-pro-image-preview` (Nano Banana Pro), `gemini-2.5-flash-image` (original Nano Banana) all show 0/0 across RPM/TPM/RPD. Lyria 3 (Clip/Pro) and Veo 3 (Generate/Fast/Lite) are also 0/0. Billing must be enabled on the GCP project to call these.
...
| `GEMINI_API_KEY`                                 | Google Imagen 4 / Gemini Flash Image | Photoreal, multimodal edits (up to 3 refs)                                  | Imagen 4 free 25 RPD per variant (~75/day); Nano Banana paid only |
