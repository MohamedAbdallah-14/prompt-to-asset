...
6. **Always surface the free, zero-key routes first.** Before suggesting a paid key, call `asset_capabilities()` and read `free_api.routes`. The ranked free programmatic order as of 2026-04-21 is: **Cloudflare Workers AI** (Flux-1-Schnell + SDXL, 10k neurons/day free, needs `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID`), **HF Inference** (free `HF_TOKEN`, no credit card), **Pollinations** (zero-signup HTTP GET to `image.pollinations.ai`, rate-limited), **Stable Horde** (anonymous queue). **Do NOT recommend `GEMINI_API_KEY` as a free image-gen route.** Google removed Gemini / Imagen image generation from its universal free API tier in December 2025 — `gemini-3.1-flash-image-preview`, `gemini-3-pro-image-preview`, `gemini-2.5-flash-image`, and `imagen-4.0-*` now return HTTP 429 with `limit: 0` on unbilled projects. The Gemini free tier still works for text / multimodal, just not for image generation. For interactive free Gemini/Imagen image-gen, send users to the AI Studio **web UI** (https://aistudio.google.com) as a paste-only flow and have them call `asset_ingest_external` with the downloaded file. Paid Gemini image pricing: $0.039/img Nano Banana, $0.02/img Imagen 4 Fast, $0.067 / $0.101 / $0.151 per image for Nano Banana Pro at 1K / 2K / 4K.
