...
6. **Always surface the free, zero-key routes first.** Before suggesting a paid key, call `asset_capabilities()` and read `free_api.routes`. The ranked free programmatic order as of 2026-04-22 is: **Cloudflare Workers AI** (Flux-1-Schnell + SDXL, 10k neurons/day free, needs `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID`), **HF Inference** (free `HF_TOKEN`, no credit card), **Pollinations** (zero-signup HTTP GET to `image.pollinations.ai`, rate-limited), **Stable Horde** (anonymous queue). **Do NOT recommend `GEMINI_API_KEY` as a free image-gen route.** Per Google's own pricing page ([ai.google.dev/gemini-api/docs/pricing](https://ai.google.dev/gemini-api/docs/pricing), verified Apr 2026), `gemini-3.1-flash-image-preview`, `gemini-3-pro-image-preview`, `gemini-2.5-flash-image`, and every `imagen-4.0-*` variant show `Free Tier: Not available`. An unbilled `GEMINI_API_KEY` returns HTTP 429 `limit: 0` on image endpoints. The Gemini free tier still covers text / multimodal understanding / embeddings subject to RPD/RPM/TPM caps (viewable in AI Studio), just not image generation. For free Gemini/Imagen image-gen, either send users to the AI Studio web UI (https://aistudio.google.com — 500–1,000 images/day dynamic, no credit card) as a paste-only flow and call `asset_ingest_external`, or point them to the Gemini consumer app (https://gemini.google.com — Basic 20 img/day, AI Plus 50/day, AI Pro 100/day, Ultra 1,000/day per Google's help page, Mar 2026). Paid per-image pricing: original Nano Banana `gemini-2.5-flash-image` $0.039; Nano Banana 2 `gemini-3.1-flash-image-preview` $0.045/0.5K, $0.067/1K, $0.101/2K, $0.151/4K; Nano Banana Pro `gemini-3-pro-image-preview` **$0.134/1K-2K, $0.24/4K** plus $0.0011 per input image; Imagen 4 Fast $0.02, Standard $0.04, Ultra $0.06. Batch API is 50% off.
...
| **`api` (free)** | **No** | Automation without a paid key | Routes to Cloudflare Workers AI (`cloudflare-flux-schnell`, `cloudflare-sdxl` — 10k neurons/day free), HF Inference (`hf-sdxl`, `hf-sd3`, `hf-flux-schnell` — free `HF_TOKEN`), Pollinations (`pollinations-flux` et al. — zero-signup HTTP GET), or Stable Horde (`horde-sdxl`, `horde-flux` — anonymous queue). Rate-limited; Pollinations is RGB-only. **Gemini/Imagen are NOT in this list** — no free API tier since 2025-12. Same pipeline as paid api. |
...
- If no key at all: prefer `inline_svg` mode for logos/favicons/icons, OR point the user at Cloudflare Workers AI (free 10k neurons/day — needs `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID`), OR Pollinations (`curl -o out.png "https://image.pollinations.ai/prompt/<urlencoded>?model=flux&nologo=true"`), OR send them to the AI Studio **web UI** (https://aistudio.google.com) for free interactive Gemini/Imagen image generation — they download the PNG and call `asset_ingest_external`. **Do NOT tell users a `GEMINI_API_KEY` gives them free image generation — Google removed the image-gen free API tier in December 2025.**
