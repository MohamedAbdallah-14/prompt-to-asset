...
export async function doctorCommand(): Promise<void> {
  ...
  lines.push("Free-tier / zero-key routes  (recommended starting point — $0)");
  for (const name of FREE_TIER_PROVIDERS) {
    const live = avail[name];
    const label = freeHint(name);
    lines.push(`  ${name.padEnd(14)} ${live ? "on " : "off"}  ${label}`);
  }
  lines.push(
    "  google         free tier via GEMINI_API_KEY — ~1,500 images/day on Gemini 3 Flash Image"
  );
  lines.push(
  ...
  if (!anyPaid) {
    ...
    lines.push("  You are fully operational for free. Try:");
    lines.push(`    p2a models list --free           # see zero-key / free-tier options`);
    lines.push(`    export GEMINI_API_KEY=...        # best free tier (Nano Banana, ~1.5k/day)`);
    lines.push(
      `    curl -o logo.png "https://image.pollinations.ai/prompt/minimal+flat+vector+logo?model=flux&nologo=true"`
    );
    lines.push("");
  }
