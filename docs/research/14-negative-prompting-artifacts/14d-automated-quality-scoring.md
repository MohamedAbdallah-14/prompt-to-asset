...
- **Paper:** [arXiv:2508.03789](https://arxiv.org/abs/2508.03789) · **ICCV 2025** poster.
- **Dataset:** **HPDv3** — **1.08M text–image pairs, 1.17M pairwise annotations**, advertised as the first "wide-spectrum" preference dataset: mixes SOTA generator outputs **and real-world images of varying quality**.
- **Architecture:** **VLM-based** preference model (supersedes CLIP/BLIP feature extractors used by v1/v2 and ImageReward) with an **uncertainty-aware ranking loss** that accounts for annotator disagreement.
...
4. **Self-preference.** VLMs tend to rate output generated by models in their own family higher. GPT-4o slightly prefers `gpt-image-1` outputs; Gemini prefers Imagen. **Mitigation:** cross-family ensemble (GPT-4o + Gemini 2.5 Pro + Claude 4.5 Sonnet).
...
│  or fan out to 2-3) │       Flux pro, Recraft v3
│   N = 4..8 samples  │
└─────────┬───────────┘
          │
          ▼
