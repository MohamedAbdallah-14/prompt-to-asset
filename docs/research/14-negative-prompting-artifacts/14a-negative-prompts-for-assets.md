...
- **The model-support matrix has four tiers, not two.** (1) *Native two-pass negative prompt* — SDXL, SD 1.5, SD3, Ideogram (API-level), Vertex Imagen (partial). (2) *Single-axis exclusion operator* — Midjourney `--no` and `::-0.5` multi-prompts, Recraft `negative_prompt`. (3) *No negative prompt at all* — Flux.1-dev/schnell, DALL·E 3, `gpt-image-1`, Gemini 2.5 Flash Image ("Nano Banana"), most video models. (4) *Partial / undocumented* — Flux with community `pipeline_flux_with_cfg` re-introducing real CFG at 2× cost.
...
| **SD3 Medium / SD3.5** | **Yes** | Triple text encoders; `negative_prompt` supported. Negative-token influence is weaker than SDXL in practice — bias enhancer toward stronger emphasis. | Use negatives but keep them short; SD3 text encoders dilute long lists. |
...
| **FLUX.1-pro / Flux Kontext** (API) | **No** | Same architecture as dev; API does not expose a negative prompt field. | Positive framing only. |
| **Midjourney v6 / v6.1 / v7** | **Yes (operator)** | `--no token1, token2` or inline `::-0.5` multi-prompt weights. `--no` is functionally equivalent to weighing a multi-prompt element at −0.5 per the docs. | Translate the SDXL-style negative list into `--no` CSV; watch for MJ's per-word moderation parsing. |
| **Ideogram 2.0 / 3.0** | **Yes (API)** | `negative_prompt` string in the `/generate-v3` request. Official docs warn that "descriptions in the prompt take precedence" and that the model "struggles with understanding negation" — prefer positive framing when possible. | Use for the long-tail artifacts (watermark, jpeg artifacts). Keep the list short and concrete. |
| **Recraft v2 / v3** | **Yes (partial)** | API exposes `negative_prompt` on raster endpoints; vector endpoints largely ignore it (vector model reshapes primitives). | Use for raster style modes only; for SVG, lean on style tokens. |
| **DALL·E 3 / `gpt-image-1` / `gpt-image-1.5`** | **No** | OpenAI Images API has no `negative_prompt` field; DALL·E 3 additionally *rewrites* the user's prompt server-side, which can re-introduce artifacts the user tried to exclude. | Positive framing; add terse "do not" phrases *inside* the natural-language prompt only as a last resort. |
| **Gemini 2.5 Flash Image ("Nano Banana") / Imagen 4** | **No** (Gemini side); **Deprecated** (Imagen 4 on Vertex) | Vertex Imagen 2/3 accepted `negativePrompt`; Imagen 4 and Gemini 2.5 Flash Image do not. | Positive framing only. |
...
Per Ideogram's own prompting guide, keep the `negative_prompt` list short and concrete — "descriptions in the prompt take precedence."
...
15. [Vertex AI: *Generate Images with Imagen*](https://cloud.google.com/vertex-ai/generative-ai/docs/image/generate-images) — Imagen 2/3 `negativePrompt`; Imagen 4 / Gemini 2.5 Flash Image do not expose it.
