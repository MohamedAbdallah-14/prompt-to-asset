...
> **Updated 2026-04-21:** remove.bg pricing as of March 2026 snapshot: entry tier ~$9/40 credits/month ($0.225/image); 200-credit tier ~$39/month ($0.195/image). These differ slightly from figures in the table below which were collected at a different snapshot — treat the table as approximate order-of-magnitude; verify at [remove.bg/pricing](https://www.remove.bg/pricing) before committing to a billing plan. The 50 free API previews/month remain low-res only (no watermark but deliberately undersized for testing). Credits roll over for a maximum of five months.
...
| **Cloudinary `e_background_removal`** (on-the-fly transformation) | **75 transformation units / image** per [transformation_counts](https://cloudinary.com/documentation/transformation_counts#special_effect_calculations). ~**$0.012–0.05/image** depending on plan (Free/Plus/Advanced) | 25k monthly credits on Free plan → ~333 bg-removals/mo | up to ~**50 MP** (plan-dependent) | PNG/WebP derived asset, delivered via CDN | JPG/PNG/WebP/HEIC/TIFF → PNG/WebP/AVIF/JXL | Global CDN; first request can return **HTTP 423** while the derived asset is computed | Fully transparent PNG, cacheable on CDN; combine with `c_pad,b_transparent` etc. **The separate "AI Background Removal Add-on" subscription is deprecated — new accounts created after 2026-02-01 cannot subscribe to it**; the `e_background_removal` transformation itself continues as the supported path, billed per transformation unit. Existing add-on subscribers should migrate to transformation billing. |
...
4. **Clipdrop** (originally Stability AI; acquired by Jasper.ai in February 2024 — the background removal API at `clipdrop-api.co/remove-background/v1` remains operational) — fourth place in the Arena.
