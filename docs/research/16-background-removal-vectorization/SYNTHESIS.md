...
5. **vtracer is the production default for color vector output; potrace for 1-bit.** vtracer (Rust/wasm, MIT, ~5.8k stars) produces compact multi-color SVGs comparable to Illustrator's Image Trace and ships official `vtracer-wasm` on npm (published 2025-07) so the entire vectorizer can run in the user's browser in a Web Worker. potrace remains geometrically cleaner on 1-bit bitmaps (wordmarks, monochrome icons) but is GPL and lacks a first-party wasm build (16c).
...
| **16c** | Vectorization tooling: vtracer vs potrace vs imagetracerjs vs autotrace; WASM ports; parameter cheatsheet; segment-then-trace recipe; SVGO cleanup; why DiffVG fails in production | `vtracer`, `vtracer-wasm`, `potrace`, `imagetracerjs`, `SVGO` | Frontend + backend engineers shipping SVG logos/icons |
...
7. **In-browser vectorization via `vtracer-wasm`** (npm, first-party, July 2025) inside a dedicated Web Worker. Lazy-load the ~1–2 MB wasm bundle only when the user requests vectorize; do not put it on the critical path. (16c)
...
- [`visioncortex/vtracer`](https://github.com/visioncortex/vtracer) + [`vtracer-wasm` npm](https://www.npmjs.com/package/vtracer-wasm)
