...
5. **Celebrity and public-figure names are stripped** even when the user asks for the prompt to be preserved verbatim. This is a separate, earlier pipeline stage from the "I NEED" rewriter suppression — the suppression trick does not affect it.
...
9. **Plan the May 2026 sunset now.** `dall-e-3` is deprecated 2026-05-12. Build your prompt-to-asset to emit an intermediate representation (subject + style + palette + composition + size + quality + negatives) and compile to each backend — `dall-e-3` (`style`/`quality`), `gpt-image-1.5` (`quality: low|medium|high`, `background`), `imagen-4`, `gemini-3-pro-image`, Recraft, Flux. Hard-coding dall-e-3 parameter names guarantees churn.
