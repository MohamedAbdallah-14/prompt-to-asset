...
7. **Recraft v3 is the only major hosted API with first-class palette input.** `extra_body.controls.colors` accepts 2–5 RGB triplets and, combined with `style="vector_illustration"` or a custom `style_id`, is the strongest single generation-time signal for palette compliance ([15c §1.1](./15c-brand-color-palette-enforcement.md)). Every other model requires either IP-Adapter swatch conditioning (SDXL/Flux, ΔE ≈ 5–15) or pure post-process remap.
...
15. **Drift has three channels, each with its own mitigation.** (a) Model version drift (MJ sref codes change across versions) — pin versions (`--v 6 --sv 4`) and diff golden sets per release. (b) Operator drift (different strengths applied by different people) — centralize `--sw` / IP-Adapter weight / LoRA scale in `model_bindings`. (c) Content leakage (reference subject in outputs) — use InstantStyle / VSP / B-LoRA style-only, subject-light moodboards, explicit subject negatives ([15b §Failure modes](./15b-style-transfer-sref-b-lora.md), [15a §Drift Mitigation](./15a-consistent-character-and-mascot.md)).

## Map of the Angles

| Angle | Scope | Key primitive | Where it sits in the bundle |
|---|---|---|---|
| **15a** — Consistent character/mascot | Subject identity across scenes (people, mascots, products) | DreamBooth-LoRA, IP-Adapter-FaceID, InstantID, PhotoMaker, PuLID, MJ `--cref`, Ideogram char-ref, gpt-image-1 multi-image, Gemini 2.5 Flash conversational | `$extensions.ai.generation.loras[]`, `character_references[]`, `seed_library.json` |
...
2. **Structured-parameter layer** — model-specific flags routed by `model_bindings.<model>`: MJ gets `--sref <code> --sw 250..400 --ar 1:1`, Flux/SDXL get `<lora:brand_v3:0.75>` + IP-Adapter reference at weight 0.6, Ideogram gets `style_codes` OR `style_reference_images` (they are mutually exclusive per the API), Recraft gets `style_id` plus `controls.colors`, Gemini/GPT-Image have no structured channel so Layer 2 folds back into Layer 1.
