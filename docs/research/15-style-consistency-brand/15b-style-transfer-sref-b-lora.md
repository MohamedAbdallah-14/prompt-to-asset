...
| **Flux.1 dev/pro** | Style via LoRA (no native style-ref param yet in most API wrappers) | Community Flux-IP-Adapter and Flux style LoRAs | `lora_scale=0.6–1.0` | Cleaner base model; stronger text rendering than SDXL |
...
- **Recraft v4 caveat:** at the time of writing styles are v2/v3 only. If you need v4's improved quality, you lose the style system and must rely on detailed prompt language instead.
...
3. **Model update rug-pull** — MJ silently changes what a given sref code means in a minor version. Remedy: pin `--v 6 --sv 4` for legacy anchors; regenerate canonical samples after every major release and diff against previous via CSD.
