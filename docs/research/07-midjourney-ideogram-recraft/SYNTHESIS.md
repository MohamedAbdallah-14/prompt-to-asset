...
4. **Midjourney is great and almost useless for automation.** v7's aesthetic, `--sref` style catalog, and `--oref` omni-reference are unmatched for concept boards — but Midjourney still has no official API, only Discord/web UIs and ToS-violating third-party bridges (Apify, useapi, ImagineAPI, Legnext). A production prompt-to-asset cannot cleanly integrate MJ server-side ([7a](./7a-midjourney-v6-v7-prompting.md)). Treat it as a human-in-the-loop surface that the enhancer *emits prompts for*, not *calls*.
...
6. **Recraft V4 removed styles; V3 is the architectural default.** The 2025/2026 successor `recraftv4` / `recraftv4_vector` / `recraftv4_pro` does **not** support `style` or `style_id` — the feature that makes Recraft worth integrating. Any brand-style pipeline must pin to `recraftv3` / `recraftv3_vector` / `recraftv2` and opt into V4 only for tasks that don't need a brand token ([7c](./7c-recraft-v3-vector-and-brand-styles.md)).
