...
1. **FLUX.1 [dev] is non-commercial by default, but a self-serve commercial license is now available.** The base weights ship under the `flux-1-dev-non-commercial-license` v1.1.1 / v2.0 ([LICENSE-FLUX1-dev](https://github.com/black-forest-labs/flux/blob/main/model_licenses/LICENSE-FLUX1-dev), [BFL legal](https://bfl.ai/legal/non-commercial-license-terms)). Every LoRA trained on top inherits that restriction at inference time, regardless of the adapter's own license tag. Outputs are *not* restricted derivatives, but serving the combined model to paying end users is prohibited **without a commercial license**. > **Updated 2026-04-21:** BFL launched a self-serve commercial license portal (`bfl.ai/licensing`) — $999/month base for up to 100k images, $0.01/image above that. Self-hosted commercial deployment of Flux [dev] LoRAs is no longer blocked behind a custom enterprise deal.
...
> **Updated 2026-04-21:** BFL now offers a **self-serve commercial license** for FLUX.1 [dev] at $999/month (includes up to 100k images/month; $0.01/image above that) via a self-serve portal at `bfl.ai/licensing`. This means commercial self-hosted deployment of Flux [dev] LoRAs is now available without a custom enterprise deal — the $999 flat is viable for production pipelines generating >30k images/month (cheaper than fal/Replicate at that volume). Below that threshold, fal's FLUX.1-dev hosted endpoint remains cheaper per-image.
