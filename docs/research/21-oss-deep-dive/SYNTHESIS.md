...
10. **`@realfavicongenerator/generate-favicon` is a strong pick for the favicon niche** — the actual engine behind `realfavicongenerator.net`, released as an MIT TypeScript monorepo in April 2024, now ~5k★ (significantly grown since initial discovery; v0.8.0 April 2026). Ships its own linter. The modern ecosystem has standardized on `sharp` + `resvg` + `png-to-ico`; every post-2024 CLI bans Puppeteer/ImageMagick/Inkscape. `itgalaxy/favicons` is also actively maintained at v7.1.3 (April 2026). `pwa-asset-generator` moved to `elegantapp/pwa-asset-generator`, now at v8.1.4 (March 2026) (see 11 §"The canonical player"; 11 §"Integration recommendations").

> **Updated 2026-04-21:** The "undiscovered, 34★" characterization is stale — the package has grown substantially to ~5k★ and is actively maintained at v0.8.0.
11. **Takumi (Rust, Apache-2.0/MIT) is to Satori what Satori was to headless Chrome** — 2–10× faster, full CSS Grid + variable fonts + COLR emoji + RTL + CJK, native `next/og`-compatible API. Nuxt OG Image v6 *recommends* Takumi as the primary renderer (renderer selection is per-component via filename suffix, not a global default — v6 removed `defaults.renderer`). For Cloudflare Workers the path is Takumi WASM primary + `workers-og` fallback; headless Chrome is ~99% more expensive at OG scale (see 12 §"The new frontrunner"; 12 §"Integration recommendations").

> **Updated 2026-04-21:** "Nuxt OG Image v6 defaults to Takumi" was a simplification — v6 recommends Takumi via `.takumi.vue` suffix convention, not a single global default setting.
...
14. **Skills-packaging convergence is real but shallow:** `SKILL.md` body is portable across Claude Code, Cursor, Windsurf, Codex, Gemini CLI (via `contextFileName` glob); `AGENTS.md` is the cross-vendor LCD (Linux Foundation / AAIF stewardship since Dec 2025, now 170+ member orgs as of April 2026; 60k+ repos; 28.6% median runtime reduction per Princeton study). Frontmatter is compile-target-specific — `rule-porter` and `FrancyJGLisboa/agent-skill-creator` already do the stamping (see 15 §"Per-surface summary"; 15 §"Packaging recommendations").
...
| 11 | [`11-oss-favicon-generators.md`](./11-oss-favicon-generators.md) | `@realfavicongenerator/generate-favicon` (MIT, ~5k★ April 2026, v0.8.0) is the actual engine behind RFG.net; `itgalaxy/favicons` workhorse (v7.1.3 April 2026); `pwa-asset-generator` moved to `elegantapp/pwa-asset-generator` (v8.1.4 March 2026); second-gen Sharp/Bun CLIs (favsmith, pwa-icons, favpie); `astro-favicons` for framework. |
...
Flux.1 [dev], Flux.1 Redux-dev, and Flux.1 Kontext-dev are the recurring non-commercial gate **for unlicensed use**. 01 §"The license landscape" names this as the dominant decision for LoRA bundling; 04 §"Flux port" disqualifies the Flux-LayerDiffuse path for commercial deployment without a licence; 16 §"Cross-cutting notes" lists Flux.1-dev / Flux.1 Redux-dev / Flux.1 Kontext-dev / IP-Adapter-FaceID v2 / RMBG-2.0 as the silent traps; 17 §"JourneyDB" treats Midjourney prompt corpora as unlicensed derivatives. The safe commercial spine **without extra purchase** is SDXL (OpenRAIL-M) + Flux.1 [schnell] (Apache-2.0) + Qwen-Image (Apache-2.0) + hosted Flux via Together/Replicate/fal endpoint that carries its own commercial grant. **With a BFL self-serve commercial licence** (available since June 2025): Flux.1 [dev] and Flux.1 Kontext [dev] are also commercial-safe — the licence is now purchasable in minutes via `bfl.ai/licensing`.

> **Updated 2026-04-21:** BFL launched a self-serve commercial licensing portal in June 2025 for Flux.1 [dev], Flux.1 Kontext [dev], and Flux.1 Tools [dev], removing the previously opaque commercial-licence negotiation. The "no-fly list" for these models now applies only to unlicensed self-hosting.
...
16. **Deploy serverless ComfyUI as: Modal primary (`@modal.enter(snap=True)`, `min_containers=1–3` per asset class) → RunPod `worker-comfyui` fallback (A100/H200 capacity spikes) → Replicate `any-comfyui-workflow` escape hatch. Ship every workflow in `lib/workflows/` as a `bentoml/comfy-pack` `.cpack.zip` — it is the best portable lock format regardless of deploy plane** (see 18 §"Recommendation"). Disqualify ComfyICU (no custom nodes → no LayerDiffuse). ComfyDeploy is now at ~1.3k★ (Sep 2025 re-open-sourcing validated); comfy-pack is at ~164★ and pre-release cadence — still the best lock-format design regardless of star count.
...
19. **Gate every commercial route on licence provenance: Apache-2.0 / MIT / CC0 / OpenRAIL-M is ship-safe; IP-Adapter-FaceID-v2 weights / RMBG-2.0 weights / JourneyDB / Logo-2K+ / METU / LLD / unDraw-scraping remain the no-fly list for unlicensed use. Flux.1 [dev] / Flux.1 Kontext-dev can now be commercially licensed via BFL's self-serve portal (`bfl.ai/licensing`, June 2025) — treat them as "commercial OK with paid BFL licence." Route "premium" through paid APIs carrying their own commercial grant (Together, Replicate, fal, BFL hosted, Ideogram, Recraft, OpenAI)** (see 01 §"The license landscape"; 16 §"Cross-cutting notes"; 17 §"Integration recommendations"; 20 §"Prior art").

> **Updated 2026-04-21:** Flux.1 [dev] and Flux.1 Kontext [dev] removed from the no-fly list for organisations that purchase a BFL commercial licence. The self-serve portal launched June 2025 and covers Flux.1 [dev], Flux.1 Kontext [dev], and Flux.1 Tools [dev]. Flux.1 Redux-dev commercial licensing status not separately confirmed — verify at `bfl.ai/licensing` before use.
