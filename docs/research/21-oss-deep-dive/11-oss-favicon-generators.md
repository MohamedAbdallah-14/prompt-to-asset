...
- **`onderceylan/pwa-asset-generator`** — 3,006★, MIT. Puppeteer-based (pulls Chromium ~200 MB), tracks Apple's splash-screen matrix best. Include only for `pwaSplash` opt-in.
...
1. **`@realfavicongenerator/generate-favicon` is the pick-one-and-done answer** — it reproduces SaaS output deterministically from local Sharp with zero network calls, and ships its own linter. It just hasn't been discovered yet (34★ vs. `favicons`' 1,234★).
