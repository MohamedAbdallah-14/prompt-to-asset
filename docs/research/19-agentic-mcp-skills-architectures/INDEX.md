...
4. **The MCP spec has collapsed to "stdio local, Streamable HTTP remote".** Plain HTTP+SSE is deprecated as of 2025-03-26. New servers must target spec **2025-06-18** minimum (stable), optionally negotiate **2025-11-25** (draft). `structuredContent` + `outputSchema` are now the default way to return typed data; avoid base64 blobs — return MCP **resource URIs** for large assets (`19b`).
...
7. **Only Claude Code and Codex expose real lifecycle hooks.** `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse` exist in both (with minor renames: Claude's `timeout` → Codex's `timeoutMs`, Claude's shell `command` → Codex's split `command`+`args`). Cursor has a separate `hooks.json` contract. **Windsurf, Cline, and Gemini CLI have no lifecycle hooks** — on those surfaces, enforcement is prose (`19a`, `19c`).
...
**Enforce-via-hooks in Claude/Codex; enforce-via-prose everywhere else.** `19a`'s compatibility matrix, `19c`'s trust table, and `19e`'s installer design all point at the same asymmetry: Claude Code and Codex support `SessionStart`/`UserPromptSubmit`/`PreToolUse` hooks that can *block* bad behavior; Cursor has partial support via a separate `hooks.json`; Windsurf/Cline/Gemini CLI have none. The prompt-to-asset therefore cannot assume enforcement — it can only assume *suggestion*. For the three IDEs without hooks, the always-on rule body plus strong `SKILL.md` descriptions are the entire enforcement surface.
...
**The rewriter is small, specialised, and replaceable.** `19d` (Hunyuan PromptEnhancer, PromptSculptor, Anthropic prompt improver) and `19b` (prompts as a first-class MCP primitive) agree: the enhancement LLM itself should be a Haiku/Flash-class model called with structured JSON output, not Opus. The graph is designed so a fine-tuned local rewriter can slot in later without changing the surrounding orchestration.
...
- **Gemini CLI has no lifecycle hooks yet.** There is no way to fire code on `SessionStart` or `UserPromptSubmit`. `19a` and `19c` both note this; the project must accept pure prose as enforcement for that surface or contribute upstream.
